...
def encode_corpus(corpus):
    ...
    for batch in tqdm(dataloader):
        batch = move_to_cuda(batch, args.device)
...
def flask():
    corpus_doc = request.json['corpus']
    corpus_doc_emb = encode_corpus(corpus_doc)

    index.add(corpus_doc_emb)
    corpus.extend([_['text'] for _ in corpus_doc])

    print(index.ntotal, index.d)

    return jsonify({'message': 'Doc sentence embedding ready!'})


@app.route('/flask/ask_doc', methods=['POST'])
def ask_doc():
    ...
    sims, indices = index.search(q_emb, 10)

    context_1 = [corpus[idx] for idx in indices[0]]
    sims_1 = sims[0]
    ...
    for i, row in enumerate(indices):
        for j, idx in enumerate(row):
            ...
            context_2.append([context_1[i], corpus[idx]])
    ...
    top_k = np.argsort(scores_2)[::-1][:min(15, len(scores_2))]
    res = []
    for idx in top_k:
